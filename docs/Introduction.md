...
## Optimization Template for XBraid_Adjoint {#optimization} 
   ...
   In order to use this template, additional interface routines are required, most importantly, the user implements a `DesignUpdate' routine which performes an update to the design \f$\rho\f$, based on the gradient information provided by XBraid_Adjoint. Here, the user can implement his favorite Hessian approximation in order to precondition the design iteration. 
