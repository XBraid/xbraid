...
f_lorenz_du(const VEC& u)
{
  ...
  MAT out;
  out << -sigma, sigma, 0, rho - z, -1, -x, y, x, -beta;
  return out;
}

VEC
euler(VEC u, double dt)
{
  return u + dt * f_lorenz(u);
}

MAT
euler_du(VEC u, double dt)
{
       ...
       double tol)
{
  ...
  if (P_tan)
    {
      A = MAT::Identity() - dt * (1 - theta) * f_lorenz_du(ustop);
      *P_tan = A.partialPivLu().solve(MAT::Identity()
                                      + dt * theta * f_lorenz_du(u));
    }
  ...
  for (int i = 0; i < newton_iters; i++)
    {
      ...
      rhs << k.head(DIM) - dt * f_lorenz(u1), k.tail(DIM) - dt * f_lorenz(u2);
      ...
      A << MAT::Identity() - dt * a11 * B1, -dt * a12 * B1, -dt * a21 * B2,
          MAT::Identity() - dt * a22 * B2;
  ...
  if (P_tan)
    {
      ...
      RHS << K1, K2;
      A << MAT::Identity() - a11 * K1, -a12 * K1, -a21 * K2,
          MAT::Identity() - a22 * K2;
  ...
  if (P_tan)
    {
      ...
      K3 = dt * f_lorenz_du(u2)
           * (MAT::Identity() + (1 - theta) / 4 * K1 + (1 + theta) / 4 * K2);
      K4 = dt * f_lorenz_du(u3) * (MAT::Identity() + K3);
      *P_tan = MAT::Identity() + (K1 + 2 * K2 + 2 * K3 + K4) / 6;
    }
  return u + (k1 + 2 * k2 + 2 * k3 + k4) / 6;
...
Eigen::Vector<double, 3 * DIM>
lobatto3rhs(VEC u,
            Eigen::Vector<double, 3 * DIM> k,
            double dt,
            ...
            VEC& u3)
{
  ...
  u1 = u + A(0, 0) * k.head(DIM) + A(0, 1) * k.segment(DIM, DIM)
       + A(0, 2) * k.tail(DIM);
  u2 = u + A(1, 0) * k.head(DIM) + A(1, 1) * k.segment(DIM, DIM)
       + A(1, 2) * k.tail(DIM);
  u3 = u + A(2, 0) * k.head(DIM) + A(2, 1) * k.segment(DIM, DIM)
       + A(2, 2) * k.tail(DIM);

  rhs << k.head(DIM) - dt * f_lorenz(u1),
      k.segment(DIM, DIM) - dt * f_lorenz(u2), k.tail(DIM) - dt * f_lorenz(u3);
  return rhs;
}

double
line_search(double& phi_0,
            Eigen::Vector<double, 3 * DIM> p,
            Eigen::Vector<double, 3 * DIM>& k,
            Eigen::Vector<double, 3 * DIM>& rhs,
            VEC u,
            double dt,
            ...
            VEC& u3)
{
  ...
  while ((i < maxiters)
         && (phi_1 * phi_1 > phi_0 * phi_0 + 2 * c1 * alpha * phi_prime))
    {
      // approximately optimize alpha using quadratic approximation to phi
      double a_new = -alpha * phi_prime
                     / (phi_1 * phi_1 - phi_0 * phi_0 - 2 * phi_prime * alpha);
      if (a_new < gam1 * alpha)
        {
          alpha = gam1 * alpha;
        }
      ...
      if (linesearch && i > 0)
        {
          ...
          // and also overwrites k, rhs, u1, u2, u3 and norm with the latest
          // values
...
// VEC thetaLobatto3(VEC u, VEC ustop, double dt, double th_A, double th_B,
// double th_C, MAT *P_tan, int newton_iters, double tol)
// {
//     const int stages = 3;
...
//     {
//         Eigen::Vector<double, stages * DIM> rhs;
//         u1 = u + a11 * k.head(DIM) + a12 * k.segment(DIM, DIM) + a13 *
//         k.tail(DIM); u2 = u + a21 * k.head(DIM) + a22 * k.segment(DIM, DIM)
//         + a23 * k.tail(DIM); u3 = u + a31 * k.head(DIM) + a32 *
//         k.segment(DIM, DIM) + a33 * k.tail(DIM);
...
//         {
//             u1 = u + a11 * k.head(DIM) + a12 * k.segment(DIM, DIM) + a13 *
//             k.tail(DIM); u2 = u + a21 * k.head(DIM) + a22 * k.segment(DIM,
//             DIM) + a23 * k.tail(DIM); u3 = u + a31 * k.head(DIM) + a32 *
//             k.segment(DIM, DIM) + a33 * k.tail(DIM);
//         }
//         Eigen::Matrix<double, stages * DIM, DIM> RHS;
//         MAT K1 = dt * f_lorenz_du(u1);
...
void
pack_array(std::ofstream& f, VEC u)
{
  f << u[0] << ',' << u[1] << ',' << u[2] << '\n';
}

void
pack_darray(std::ofstream& f, COL_MAT u)
{
  f << u(0, 0) << ',' << u(1, 0) << ',' << u(2, 0);
...
void
bf_pack_help(double* buf, const VEC u, const size_t obj_size, size_t& bf_size)
{
  size_t i_bf = bf_size;
  for (size_t i = 0; i < obj_size; i++)
    {
...
void
bf_pack_help(double* buf, const MAT u, const size_t obj_size, size_t& bf_size)
{
  size_t i_bf = bf_size;
  for (size_t i = 0; i < obj_size; i++)
    {
...
void
bf_unpack_help(double* buf, COL_MAT& u, const size_t obj_size, size_t& bf_size)
{
  size_t i_bf = bf_size;
  for (size_t i = 0; i < obj_size; i++)
    {
...
void
bf_unpack_help(double* buf, VEC& u, const size_t obj_size, size_t& bf_size)
{
  size_t i_bf = bf_size;
  for (size_t i = 0; i < obj_size; i++)
    {
...
void
bf_unpack_help(double* buf, MAT& u, const size_t obj_size, size_t& bf_size)
{
  size_t i_bf = bf_size;
  for (size_t i = 0; i < obj_size; i++)
    {
