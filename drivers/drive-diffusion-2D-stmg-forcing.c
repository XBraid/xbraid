...
/**
 * Driver:        drive-diffusion-2D-stmg-forcing.c
 *
 * Interface:     C
 * 
...
#if DEBUG
{
   ...
   if (fstop == NULL)
   {
      ...
      if (app->man->forcing)
      {
         if (app->man->explicit)
         {
            /* g_{tstop} = dt*b_{tstart} */
            bstop = u->b;
            HYPRE_SStructVectorGetObject( bstop, (void **) &sbstop );
            hypre_StructScale((tstop-tstart), sbstop);
         }
         else
         {
            /* g_{tstop} = dt*b_{tstop} */
            bstop = ustop->b;
            HYPRE_SStructVectorGetObject( bstop, (void **) &sbstop );
            hypre_StructScale((tstop-tstart), sbstop);
         }
   ...
   else
   {
      ...
      if (app->man->forcing)
      {
         if (app->man->explicit)
         {
            ...
            hypre_StructScale((tstop-tstart), sg);
            bstop = fstop->x;
            HYPRE_SStructVectorGetObject( bstop, (void **) &sbstop );
            hypre_StructAxpy (1.0, sg, sbstop);
         }
         else
         {
            /* g_{tstop} = dt*b_{tstop} */
            HYPRE_SStructVectorGetObject( ustop->b, (void **) &sg );
            hypre_StructScale((tstop-tstart), sg);
            bstop = fstop->x;
            HYPRE_SStructVectorGetObject( bstop, (void **) &sbstop );
            hypre_StructAxpy (1.0, sg, sbstop);
            ...
            braid_StepStatus status)
{
   ...
   HYPRE_SStructVector g;
   HYPRE_StructVector  sg;
...
#if DEBUG
{
   ...
   if (app->man->forcing)
   {
      if (app->man->explicit) // used ???
         g = r->b;
      else
         g = ustop->b;
      HYPRE_SStructVectorGetObject( g, (void **) &sg );
      hypre_StructScale((tstop-tstart), sg);
   }
   else
      g = NULL;
      
   comp_res2(app->man, ustop->x, r->x, g, tstart, tstop);
...
/* --------------------------------------------------------------------
 * Create the shell of a vector object for a given time point, i.e.,
 * initialize the shell part (the PDE forcing values in b only).
 * -------------------------------------------------------------------- */
int
         ...
         braid_Vector *v_ptr)
{
   ...
   if (app->man->forcing)
   {
      HYPRE_SStructVectorGather( u->b );
      HYPRE_SStructVectorGetBoxValues( u->b, 0, ilower, iupper, 0, values );
      HYPRE_SStructVectorSetBoxValues( v->b, 0, ilower, iupper, 0, values );
      HYPRE_SStructVectorAssemble( v->b );
   }
...
/* --------------------------------------------------------------------
 * Create a a copy of the shell of a vector object.
 * -------------------------------------------------------------------- */
int
           ...
           braid_BufferStatus  bstatus)
{
   ...
   {
      HYPRE_SStructVectorGather( u->b );
      HYPRE_SStructVectorGetBoxValues( u->b, 0, ilower,
                                   iupper, 0, &(dbuffer[nlx*nly+1]) );
      /* Determine number of bytes actually packed */
      braid_BufferStatusSetSize( bstatus, (2*nlx*nly + 1)*sizeof(double));
   }
   else
   {
      /* Determine number of bytes actually packed */
...
int main (int argc, char *argv[])
{
   ...
   if (forcing)
      (app->buffer_size)     = (1 + 2*nlx*nly)*sizeof(double);
   else
