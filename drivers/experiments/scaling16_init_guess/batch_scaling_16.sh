...
# fixed arguments
fargs="-tf 4 -nt 16384 -cf0 16 -theta -nu 1 -niters 2 -tol 1e-5"
...
for nc in $ncores; do
   for ml in $mlevels; do
      for cf in $cfactors; do
         echo "srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta_fmg_nc${nc}_cf${cf}_ml${ml} ${ex} ${fargs} -cf ${cf} -ml ${ml} -Delta -fmg"
         ...
         srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta_fmg_nc${nc}_cf${cf}_ml${ml} ${ex} ${fargs} -cf ${cf} -ml ${ml} -Delta -fmg
