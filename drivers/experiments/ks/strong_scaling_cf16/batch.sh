   ...
   for ml in $mlevels; do
      for cf in $cfactors; do
         for rank in $ranks; do
            ...
            echo "srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta${rank}_fmg_nc${nc}_cf${cf}_ml${ml} ${ex} ${fargs} -cf ${cf} -ml ${ml} -Delta -rank ${rank} -fmg"
            srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta${rank}_nc${nc}_cf${cf}_ml${ml}     ${ex} ${fargs} -cf ${cf} -ml ${ml} -Delta -rank ${rank}
            srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta${rank}_fmg_nc${nc}_cf${cf}_ml${ml} ${ex} ${fargs} -cf ${cf} -ml ${ml} -Delta -rank ${rank} -fmg
         done
