...
# fixed arguments
fargs="-tf 4 -nt 8192 -nx 256 -cf0 16"
dargs="-theta -Delta -Deltalvl 2"
...
for nc in $ncores; do
   ...
   for rank in $ranks; do
      echo "srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta${rank}_nc${nc} ${ex} ${fargs} -ml 4 ${dargs} -rank ${rank}"
      srun -N 19 -n ${nc} -o ${outd}/${outn}_Delta${rank}_nc${nc}       ${ex} ${fargs} -ml 4 ${dargs} -rank ${rank} 
   done
done
