 ...
 * 
 *                  s.t.  d/dt u_1(t) = u_2(t) d/dt u_2(t) = -u_2(t) + c(t)
 * 
 *               with initial condition u_1(0) = 0, u_2(0) = -1 and piecewise
 *               constant control c(t).  
 *
 *               Implements a steepest-descent optimization iteration using
 *               fixed step size for design updates.   
 *
 *               This example is similar to examples/ex-04.c, but instead of
 *               using the XBraid adjoint feature, it builds two separate
 *               XBraid hierarchies -- one for the primal forward pass and
 *               another for the adjoint backwards pass.  This example
 *               additionally allows for a distributed storage of the control
 *               variable across processors in time.  In a sense, this is a
 *               more "advanced" optimization example than examples/ex-04.c
 **/
