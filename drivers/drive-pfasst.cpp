...
/*
   Using Braid to implement PFASST

   Interface:    SStructured interface (SStruct)

   Compile with: make drive-pfasst

   Help with:    drive-pfasst -help

   Sample run:   mpirun -np 8 drive-pfasst -pgrid 1 1 8 -ml 15 -nt 128 -nx 33 33 -mi 100  
...
#include "../examples/ex-pfasst-lib.cpp"
 ...
 * or tupel of time points (PFASST-like including quadrature points)
 *   x            spatial vector 
 */
        ...
        braid_StepStatus status)
{
   ...
   HYPRE_SStructVector  bstop, tau;
   HYPRE_StructVector   sb, stau;
   ...
   if (app->pfasst)
      nodes = (int)app->man->q_data->get_num_nodes() + 1;
   ...
   if( A_idx == -1.0 ){
      ...
      * discretization level */
      ...
      /*if (app->pfasst)
          Set up spatial discretization matrix.
         setUpLaplace( app->man );
      else*/
...
#if DEBUG
{
   ...
   HYPRE_SStructMatrixPrint(filename, app->A[A_idx], 0);
      ...
      setUpStructSolver( app->man, u->x[0], u->x[0] );
   ...
   if (!(app->pfasst))
   {
      ...
      take_step(app->man, ustop->x[0], bstop, u->x[0], tstart, tstop, &iters_taken);
   }
   else
   {
      ...
      for (int m = 0; m < nodes; m++)
      {
         if (level > 0)
         {
            ...
            initialize_vector(app->man, &tau);
            
            /* Set the values. */
            values = (double *) malloc( (nlx)*(nly)*sizeof(double) );
            for (int m = 0; m < nodes; m++)
            {
               HYPRE_SStructVectorGather( u->x[m] );
               HYPRE_SStructVectorGetBoxValues( u->x[m], 0, ilower, iupper, 0, values );
               HYPRE_SStructVectorSetBoxValues( v->x[m], 0, ilower, iupper, 0, values );
               HYPRE_SStructVectorAssemble( v->x[m] );
            }
            free( values );
         }
      ...
      else
      {
         ...
         for (int m = 0; m < nodes; m++ )
         {
            HYPRE_SStructVectorGetObject( bstop->x[m], (void **) &sb );
            HYPRE_SStructVectorGetObject( tau->x[m], (void **) &stau );
            
            hypre_StructAxpy (1.0, stau, sb);
         }
      }
      //take_sdc_step(app->man, ustop->x, bstop, u->x, tstart, tstop, &iters_taken);
   }
...
#if DEBUG
{
   ...
   HYPRE_SStructMatrixPrint(filename, app->A[A_idx], 0);
      ...
      setUpStructSolver( app->man, r->x[0], r->x[0] );
      app->solver[A_idx] = app->man->solver;
   ...
   /* Compute residual Ax */
   app->man->A = app->A[A_idx];
   comp_res(app->man, ustop->x[0], r->x[0], tstart, tstop);
        ...
        braid_Vector *u_ptr)
{
   ...
   if (app->pfasst)
      ...
      nodes = 1;
   u->x = (HYPRE_SStructVector *) malloc( nodes*sizeof(HYPRE_SStructVector) );
   ...
   printf("my_Init: t = %lf, initialize %d components\n", t, nodes);
   for (int m = 0; m < nodes; m++)
      set_initial_condition(app->man, &(u->x[m]), t);
         ...
         braid_Vector *v_ptr)
{
   ...
   if (app->pfasst)
      ...
      nodes = 1;
   v->x = (HYPRE_SStructVector *) malloc( nodes*sizeof(HYPRE_SStructVector) );

   /* Create an empty vector object. */
   app->man->grid_x = (app->spatial_lookup_table[u->spatial_disc_idx]).grid_x_matrix;
   for (int m = 0; m < nodes; m++)
      initialize_vector(app->man, &(v->x[m]));
        ...
        braid_Vector u)
{
   ...
   if (app->pfasst)
      ...
      nodes = 1;
   for (int m = 0; m < nodes; m++ )
      HYPRE_SStructVectorDestroy( u->x[m] );
   free( u );
       ...
       braid_Vector y)
{
   ...
   for (int m = 0; m < nodes; m++ )
   {
      HYPRE_SStructVectorGetObject( x->x[m], (void **) &sx );
      HYPRE_SStructVectorGetObject( y->x[m], (void **) &sy );
          ...
          braid_AccessStatus  astatus)
{
   ...
   if( level == 0 )
   {
      ...
      compute_disc_err(app->man, u->x[0], t, app->e, &disc_err);
   ...
   if( app->man->output_vis && (level == 0) && (t == app->man->tstop) )
   {
      ...
      output_vis(app->man, u->x[0], t, filename_mesh, filename_err, filename_sol);
   }
...
my_SpatialNorm(braid_App     app,
               ...
               double       *norm_ptr)
{
   /* only for time point and not quadrature point? */
   norm(u->x[0], norm_ptr);
   return 0;
}
           ...
           braid_BufferStatus  bstatus)
{
   ...
   /* Grab spatial info about vector */
   int ilower[2], iupper[2], nlx, nly, nx, ny, nodes;
   double dx, dy;
   grab_vec_spatial_info(u, app->spatial_lookup_table, ilower, iupper, 
                     &nlx, &nly, &nx, &ny, &dx, &dy);
   ...
   {
      HYPRE_SStructVectorGather( u->x[m] );
      HYPRE_SStructVectorGetBoxValues( u->x[m], 0, ilower,
                                      iupper, 0, &(dbuffer[m*nlx*nly+1]) );

   }
   /* Determine number of bytes actually packed */
   braid_BufferStatusSetSize( bstatus, (nodes*nlx*nly + 1)*sizeof(double));
             ...
             braid_BufferStatus  bstatus)
{
   ...
   for (int m = 0; m < nodes; m++)
   {
      initialize_vector(app->man, &(u->x[m]));
      /* Set the values. */
      HYPRE_SStructVectorSetBoxValues( u->x[m], 0, ilower, iupper, 0, &(dbuffer[m*nlx*nly+1]) );
      HYPRE_SStructVectorAssemble( u->x[m] );
   }
              ...
              braid_CoarsenRefStatus status)
{
   ...
   /* Create an empty vector object. */
   app->man->grid_x = (app->spatial_lookup_table[fspatial_disc_idx]).grid_x_matrix;
   initialize_vector(app->man, &(fu->x[0]));
   
   /* Get StructVector objects */
   HYPRE_SStructVectorGetObject( fu->x[0], (void **) &fx );
   HYPRE_SStructVectorGetObject( cu->x[0], (void **) &cx );
                ...
                braid_CoarsenRefStatus status)
{
   ...
   {
      HYPRE_SStructVectorGather( cu->x[0] );
      cvalues = (double *) malloc( (cnlx)*(cnly)*sizeof(double) );
      HYPRE_SStructVectorGetBoxValues( cu->x[0], 0, cilower,
                                       ciupper, 0, cvalues );
   ...
   initialize_vector(app->man, &(fu->x[0]));
   if( (fiupper[0] >= filower[0]) && (fiupper[1] >= filower[1])) 
   {  HYPRE_SStructVectorSetBoxValues( fu->x[0], 0, filower,
                                    fiupper, 0, fvalues ); }
   HYPRE_SStructVectorAssemble( fu->x[0] );
   fu->spatial_disc_idx = fspatial_disc_idx; 
   if( (fiupper[0] >= filower[0]) && (fiupper[1] >= filower[1])) 
   { free(fvalues); } 
   ...
   {
       wtime = MPI_Wtime();
       HYPRE_SStructVectorGather( fu->x[0] );
       hypre_StructVector *fu_struct = hypre_SStructPVectorSVector( (hypre_SStructVectorPVector((fu->x[0]),0)), 0);
   ...
   HYPRE_SStructVectorDestroy( fu->x[0] );
   free( fu );
   fu = (my_Vector *) malloc(sizeof(my_Vector));
   app->man->grid_x = (app->spatial_lookup_table[fspatial_disc_idx]).grid_x_matrix;
   initialize_vector(app->man, &(fu->x[0]));
                       ...
                       braid_CoarsenRefStatus status)
{
   ...
   initialize_vector(app->man, &(cu->x[0]));
                         ...
                         braid_CoarsenRefStatus status)
{
   ...
   if( (filower[0] <= fiupper[0]) && (filower[1] <= fiupper[1]) )
   {
      ...
      if( (cilower[0] <= ciupper[0]) && (cilower[1] <= ciupper[1]) )
      {
         ...
         HYPRE_SStructVectorSetBoxValues( cu->x[0], 0, cilower,
                                          ciupper, 0, cvalues );
   ...
   HYPRE_SStructVectorAssemble( cu->x[0] );
                    ...
                    braid_CoarsenRefStatus status)
{
   ...
   HYPRE_SStructVectorGather( fu->x[0] );
   fvalues = (double *) malloc( fnlx*fnly*sizeof(double) );
   HYPRE_SStructVectorGetBoxValues( fu->x[0], 0, filower, fiupper, 0, fvalues );
   counter = 0;
   flrow = 0;
...
int main (int argc, char *argv[])
{
   ...
   while( arg_index < argc ){
      ...
      else if( strcmp(argv[arg_index], "-pfasst") == 0 ){
         arg_index++;
         pfasst = 1;
         qnodes = atoi(argv[arg_index++]);
      }
   ...
   if (pfasst)
   {
      (app->man->q_data)  = new GaussRadau(qnodes);
      (app->man->dtau)    = (double*) malloc (qnodes*sizeof(double));
      
      printf ("PFASST with Gauss-Radau\n");
      printf ("Number of nodes: %d\n", (int)app->man->q_data->get_num_nodes());
      printf ("Quadrature points:\n");
      vector<double> gr_nodes = app->man->q_data->get_nodes();
      for (i = 0; i < qnodes; ++i)
         printf ("%f ", gr_nodes[i]);
      printf ("\n");
      printf ("dtau values:\n");
      app->man->dtau[0] = (app->man->dt)*gr_nodes[0];
      printf ("%f ", app->man->dtau[0]);
      for (i = 1; i < qnodes; ++i)
      {
         app->man->dtau[i] = (app->man->dt)*(gr_nodes[i]-gr_nodes[i-1]);
         printf ("%f ", app->man->dtau[i]);
      }
      printf ("\n");
      printf ("Q-Matrix:\n");
      pfasst::quadrature::Matrix Q = app->man->q_data->get_q_mat();
      for (int i = 0; i < Q.get_nrows(); ++i)
      {
         for (int j = 0; j < Q.get_ncols(); ++j)
            std::cout << Q(i,j) << " ";
         std::cout << std::endl;
      }
   }
