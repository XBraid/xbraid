...
function main()
    ...
    coords_x = [x for x in x_range, y in y_range, z in z_range]
    coords_y = [y for x in x_range, y in y_range, z in z_range]
    coords_z = [z for x in x_range, y in y_range, z in z_range]
    ...
    wavenumbers_norm = sqrt.(wavenumbers_x .^ 2 + wavenumbers_y .^ 2 + wavenumbers_z .^ 2)
    ...
    velocity_x = sin.(coords_x) .* cos.(coords_y) .* cos.(coords_z)
    velocity_y = -cos.(coords_x) .* sin.(coords_y) .* cos.(coords_z)
