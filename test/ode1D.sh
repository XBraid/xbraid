...
TESTS=( "$RunString -np 1 $example_dir/ex-01" \
        ...
        "$RunString -np 2 $example_dir/ex-01f -ml 2 -cf0 2 -cf 2 -pgrid 2 1" \
        "$RunString -np 4 $example_dir/ex-01f -ml 2 -cf0 2 -cf 2 -pgrid 4 1" \
        "$RunString -np 1 $example_dir/ex-01f -dt 0" \
        "$RunString -np 1 $example_dir/ex-01f -dt 1" \
        "$RunString -np 1 $example_dir/ex-01f -dt 2" \
        "$RunString -np 1 $example_dir/ex-01f -dt 2 -ml 2 -cf0 2 -cf 2" \
        "$RunString -np 1 $example_dir/ex-01f -dt 2 -ml 3 -cf0 2 -cf 2" \
        "$RunString -np 2 $example_dir/ex-01f -dt 2 -ml 2 -cf0 2 -cf 2 -pgrid 2 1" \
        "$RunString -np 2 $example_dir/ex-01f -dt 2 -ml 3 -cf0 2 -cf 2 -pgrid 2 1" )
...
#
lines_to_check="^  time steps.*|^  number of levels.*|^  iterations.*|^  residual norm.*|^Finished braid_TestAll: all tests passed successfully.*|blob"
#
