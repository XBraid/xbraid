...
#
lines_to_check="^  time steps.*|^  number of levels.*|^  max number of levels.*|^  iterations.*|^  residual norm.*|^Finished braid_TestAll: no fails detected, however some results must be|.*Braid: Temporal refinement occurred.*|^  num_syncs.*|^  tracking timings.*"
#
