...
TESTS=( "$RunString -np 1 $example_dir/ex-01" \
        ...
        "$RunString -np 5 $example_dir/ex-01-adjoint" \
        "$RunString -np 5 $example_dir/ex-01-optimization" )
...
#
lines_to_check="^  time steps.*|^  number of levels.*|^  iterations.*|^  residual norm.*|^  state   residual.*|^  adjoint residual.*|^  Objective function.*|^  Gradient norm.*|^  optimization iterations.*|^Finished braid_TestAll: no fails detected, however some results must be|.*Braid: Temporal refinement occurred.*"
#
