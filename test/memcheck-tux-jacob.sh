...
TESTS=( "MPIWRAP_DEBUG=quiet $RunString -np 1 valgrind $valgrind_opts $example_dir/drive-05 -pgrid 1 1 1 -nt 64 -ml 15 " \
        ...
        "MPIWRAP_DEBUG=quiet $RunString -np 4 valgrind $valgrind_opts $example_dir/drive-05 -pgrid 1 1 4 -nt 32 -nx 17 17 -scoarsen -expl -mi 200 -ml 20 -cf 2 -c 0.15 "  )
...
#
lines_to_check=" _warp.* | warp.* |^  time steps.*|^  number of levels.*|^  coarsening factor.*|^  num F-C relaxations.*|^  num rels on level 0.*|^  iterations.*|^spatial problem size.*|  expl.*"
#
