...
#include "../drivers/braid_pfasst.hpp"
 ...
 *   expl                use explicit discretization (1) or not (0)
...
print_simulation_manager(simulation_manager *man)
{
   ...
   int nodes;
   if (man->q_data != NULL)
      nodes = (int)man->q_data->get_num_nodes();
   else
      nodes = 1;
...
/* --------------------------------------------------------------------
 * Set up the spatial discretization matrix.
 ...
 * -------------------------------------------------------------------- */
void
setUpLaplace( simulation_manager *man)
{
   ...
   if( (ilower[0] <= iupper[0]) && (ilower[1] <= iupper[1]) )
   {
      ...
      for( m = 0, j = 0; j < nly; j++ )
         for( i = 0; i < nlx; i++, m+=nentries ){
            for( idx = 1; idx <= 2; idx++ )
               values[m+idx] = -K/(dx*dx);
            for( idx = 3; idx <= 4; idx++ )
               values[m+idx] = -K/(dy*dy);
            
            values[m] = 2*K*( (1.0/(dx*dx)) + (1.0/(dy*dy)) );
         }
              ...
              int                 *iters_taken) /* if implicit, returns the number of iters taken */
{
   ...
   int      expl         = man->expl;         /* if true, use explicit, else implicit */
...
/* --------------------------------------------------------------------
 * SDC Time integration routine
 * -------------------------------------------------------------------- */
int take_sdc_step(simulation_manager * man,         /* manager holding basic sim info */
                  HYPRE_SStructVector  xstop,       /* approximation at tstop */
                  HYPRE_SStructVector  bstop,       /* additional RHS forcing */
                  HYPRE_SStructVector  x,           /* vector to evolve */
                  double               tstart,      /* evolve x from tstart to tstop */
