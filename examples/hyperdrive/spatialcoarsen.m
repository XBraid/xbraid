% 
% Syntax:
% spatialcoarsen(cfl, alpha, order, intorder)
%
% Input:
...
function spatialcoarsen(cfl, alpha, order, intorder)
...
% set up the vecotrs for the spectral radius od D1, D2, D1+D2
kappa1 = zeros(nk,1);
kappa2 = zeros(nk,1);
kappasum = zeros(nk,1);
...
lac = cfl*(iu*sin(2*xi) - alpha*(2*sin(2*0.5*xi)).^2);
...
% coarse grid in time and space (keeping cfl constant), doubling xi
% should we use the 2nd order discretization on the coarse grid?
% 2nd order on coarse grid
%lac = cfl*(iu*sin(2*xi) - alpha*(2*sin(2*0.5*xi)).^2);
% this is the 4th order discretization
lac = cfl*iu*sin(2*xi).*(1 + 2/3*sin(2*0.5*xi).^2) - cfl*alpha*(2*sin(2*0.5*xi).^4);
...
% plot the result
printf("Spatial order=%i, cfl=%e, alpha=%e, max spec-radius(D1+D2)=%e\n", order, cfl, alpha, max(kappasum));
h=plot(xi/pi, abs(phic), "g", xi/pi, abs(phi), "b", ...
     xi/pi, abs(phipi), "r", xi/pi, t1+t2, "c", xi/pi, kappasum, "k");
set(h,"linewidth",2);
axis([-0.25 0.75 -.1 2]);
legend("abs(phic)","abs(phi)", ...
       "abs(phipi)", "spec(D1)+spec(D2)", "spec(D1+D2)"); 
set(gca,"fontsize",18);
%pause;
return;
