...
/* currently not using accuracy or rfact_ */
   ...
   int i, stage;
   double tstage, tbc, bdataL, bdataR;   
...
/* ! 4 stages of RK-4 */
      ...
      {
         tstage = t + alpha(stage)*kd_->dt;
/*  impose bc on 'eval' grid function  */
         if (kd_->taylorbc == 1)
         {
            tbc = t;
            twbndry1( 0.0, &bdataL, kd_->L, &bdataR, stage, tbc, kd_->dt, kd_->amp, kd_->ph, kd_->om, kd_->pnr);
         }
         else if (kd_->taylorbc == 0)
         {
/* evaluate exact boundary data for the stage */
/* set stage==1 to evaluate boundary data at t=tstage */
            twbndry1( 0.0, &bdataL, kd_->L, &bdataR, 1, tstage, kd_->dt, kd_->amp, kd_->ph, kd_->om, kd_->pnr);
         }
...
/* enforce bc for the 'eval' grid function */
         bckreiss1( kd_->n, kd_->eval, bdataL, bdataR, kd_->betapcoeff, kd_->h, kd_->bcnr_ );

/* evaluate dwdt */
         dwdtkreiss1( kd_->n, kd_->eval, kd_->rhs, kd_->h, kd_->nb, kd_->wb, kd_->bop_, kd_->bope_, kd_->gh );
         
         twforce1( kd_->n, kd_->force, tstage, kd_->h, kd_->amp, kd_->ph, kd_->om, kd_->pnr, kd_->L );
...
/* compute next stage grid function 'eval'*/
            for (i=1; i<=kd_->n; i++)
            {
               kd_->eval[i] = kd_->current[i] + kd_->dt*alpha(stage+1)*(kd_->rhs[i] + kd_->force[i]);
            }
/* compute next stage boundary data */
            for (i=1; i<=3; i++)
            {
               veval(i) = vcur(i) + kd_->dt*alpha(stage+1)*dvdt(i);
            }
         }
...
/* accumulate stage contributions to the solution at next time step */
         for (i=1; i<=kd_->n; i++)
         {
            gf_->sol[i] = gf_->sol[i] + kd_->dt*beta(stage)*( kd_->rhs[i] + kd_->force[i] );
         }
         
/* accumulate bndry data */
         for (i=1; i<=3; i++)
         {
            vsol(i) = vsol(i) + kd_->dt*beta(stage)*dvdt(i);
         }
